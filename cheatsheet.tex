...
\begin{center}
\begin{tabular}{|l*5{|>{\begin{math}\displaystyle}c<{\end{math}}}|}
  \hline &&&&& \\[-2ex]
  ...
  Uniform$\{a,\ldots,b\}$ & \punifd & \dunifd & 
  \frac{a+b}{2} & \frac{(b-a+1)^2-1}{12} &
  \frac{e^{as}-e^{-(b+1)s}}{s(b-a)} \\[3ex]

  Bernoulli($p$) & \pbern & \dbern &
  p & p(1-p) & 
  1-p+pe^s \\[3ex]

  Binomial($n,p$) & I_{1-p}(n-x,x+1) & \dbin &
  np & np(1-p) &
  (1-p+pe^s)^n \\[3ex]

  Multinomial($n,p$) & & \dmult \quad \sum_{i=1}^k x_i = n&
  np_i & np_i(1-p_i) &
  ...
  Negative Binomial($r,p$) & \pnbin & \dnbin &
  r\frac{1-p}{p} & r\frac{1-p}{p^2} &
  \left(\frac{p}{1-(1-p)e^s}\right)^r \\[3ex]

  Geometric($p$) & \pgeo \quad x\in\mathbb N^+ & \dgeo \quad x\in\mathbb N^+ &
  \frac{1}{p} & \frac{1-p}{p^2} &
  \frac{p}{1-(1-p)e^s} \\[3ex]

  Poisson($\lambda$) & \ppois & \dpois &
  \lambda & \lambda &
  e^{\lambda(e^s-1)}\\[3ex]
...
\begin{tabular}{|l*5{|>{\begin{math}\displaystyle}c<{\end{math}}}|}
  ...
  Pareto($x_m, \alpha$) & 1 - \left(\frac{x_m}{x} \right)^\alpha \; x\ge x_m & 
...
\footnotetext{$\gamma(s,x)$ in the CDF of the Gamma distribution denotes the
\emph{lower incomplete gamma function} which is defined as $\gamma(s,x) =
\int_0^x t^{s-1}e^{-t}\d{t}$.}

\addtocounter{footnote}{1}
\footnotetext{$I_x(a,b)$ in the CDF of the Beta distribution denotes the
\emph{regularized incomplete beta function} and is defined as $I_x(a,b) =
B(x;a,b)/B(a,b)$, where $B(x;a,b)$ is the \emph{incomplete beta function} and
is defined as $\mathrm{B}(x; a,b) = \int_0^x t^{a-1}(1-t)^{b-1}\d{t}$.}
...
\begin{itemize}
  ...
  \item The Wald test for testing \hyp{\beta_1=0}{\beta_1\neq 0} is: reject
...
\begin{center}
  ...
  \hline &&\\[-1.5ex]
  $k$ out of $n$ & \text{w/o replacement} & \text{w/ replacement} \\[1ex]
  \hline
